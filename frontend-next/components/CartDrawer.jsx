...
function CartItem({ item }) {
  ...
  return (
    <div className="flex items-center gap-4 py-4 border-b border-gray-100 dark:border-navy-800">
      ...
      <div className="flex-grow min-w-0">
        <p className="font-semibold text-navy-900 dark:text-white text-sm leading-snug line-clamp-2">{item.name}</p>
        <p className="text-gold-600 dark:text-gold-400 font-bold text-sm mt-0.5">฿{Number(item.price).toLocaleString()}</p>
      </div>
      <div className="flex flex-col items-end gap-2 flex-shrink-0">
        ...
        <div className="flex items-center gap-1 border border-gray-200 dark:border-navy-700 rounded-lg overflow-hidden">
          <button
            onClick={() => updateQuantity(item.id, item.quantity - 1)}
            className="px-2 py-1 hover:bg-gray-100 dark:hover:bg-navy-800 transition-colors text-gray-600 dark:text-gray-300"
          >
            <Minus size={12} />
          </button>
          <span className="px-2 py-1 text-sm font-semibold min-w-[28px] text-center dark:text-white">
            {item.quantity}
          </span>
          <button
            onClick={() => updateQuantity(item.id, item.quantity + 1)}
...
export default function CartDrawer() {
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <div
        className={`fixed top-0 right-0 h-full w-full max-w-md bg-white dark:bg-navy-900 z-50 shadow-2xl flex flex-col transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
        aria-hidden={!isOpen}
      >
        {isOpen && (
          <>
            ...
            {items.length === 0 ? (
              ...
              <>
                ...
                {/* Footer */}
                <div className="border-t border-gray-100 dark:border-navy-800 bg-gray-50 dark:bg-navy-950 px-5 py-4 space-y-3">
                  ...
                  <div className="flex items-center justify-between">
                    <span className="font-semibold text-navy-900 dark:text-white">{t('cart.total')}</span>
